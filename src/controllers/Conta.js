...
const ContaController = {
    ...
    async getAccountSaldo(req, res){
        try{
            const { num } = req.params;
            const account = await Conta.getAccountSaldo(num);
            ...
            res.status(500).json({message: "Erro ao buscar saldo da conta.", error: error.message});
        }
    }
}
